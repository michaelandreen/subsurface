			    ...
			    struct filter_preset_table *filter_presets)
{
	...
	while ((end_ptr < mem.size) && (ptr = strstr(mem.buffer + end_ptr, "ZDH"))) {
		...
		if (strncmp(iter, "ZDT", 3) == 0) {
			...
			params[pnr_local] = NULL;
			ret |= parse_xml_buffer(filename, "<csv></csv>", 11, table, trips, sites, devices, filter_presets, (const char **)params);
			continue;
		}
		...
		ret |= parse_xml_buffer(filename, mem_csv.buffer, mem_csv.size, table, trips, sites, devices, filter_presets, (const char **)params);
		end_ptr += ptr - (char *)mem_csv.buffer;
		free(mem_csv.buffer);
	}
		   ...
		   struct device_table *devices, struct filter_preset_table *filter_presets)
{
	...
	if (!strcmp("DL7", csvtemplate)) {
		return parse_dan_format(filename, params, pnr, table, trips, sites, devices, filter_presets);
...
#endif
	ret = parse_xml_buffer(filename, mem.buffer, mem.size, table, trips, sites, devices, filter_presets, (const char **)params);
		      ...
		      struct device_table *devices, struct filter_preset_table *filter_presets)
{
	...
	if (parse_seabear_csv_file(filename, params, pnr, "csv", table, trips, sites, devices, filter_presets) < 0) {
		return -1;
	}
